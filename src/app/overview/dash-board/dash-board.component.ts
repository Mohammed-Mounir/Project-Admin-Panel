  ...
  ngOnInit(): void {
    ...
    this.userService.getAllUsers().subscribe(
      (res)=>{
        ...
        this.chart('Users',value,this.users.length,'column',"Average of user's Age");  
      },
      (err)=>{console.error(err)},
      ()=>{},
    )
    this.sellerService.getAllSellers().subscribe(
      (res)=>{        
        ...
        arrYears.forEach(function(x) { counts[x] = (counts[x] || 0)+1; });
        years = [
          {x :"2017", value :counts['2017'] , fill: "#24964e" , stroke:'white'},
          {x :"2018", value :counts['2018'] , fill: "#24964e" , stroke:'white'},
          {x :"2019", value :counts['2019'] , fill: "#24964e" , stroke:'white'},
          {x :"2020", value :counts['2020'] , fill: "#24964e" , stroke:'white'},
          {x :"2021", value :counts['2021'] , fill: "#24964e" , stroke:'white'},
        ];
        
        this.chart('Sellers',years,this.sellers.length,'column',"Seller's year of register");
      },
      (err)=>{console.error(err)},
      ()=>{},
    )
    this.productService.getAllProducts().subscribe(
      (res)=>{        
        ...
        {
          if(product.productPrice.finalPrice>=1000 && product.productPrice.finalPrice<5000){
            two++;
          }
          else if(product.productPrice.finalPrice>=5000 && product.productPrice.finalPrice<10000){
            three++;
          }
          else if(product.productPrice.finalPrice<1000){
            one++;
          }
          else if (product.productPrice.finalPrice>=10000 && product.productPrice.finalPrice<20000)
          {
            four++;
          }
        ...
        this.chart('Products',value,this.products.length,'line',"Product's Price");
      },
      (err)=>{console.error(err)},
      ()=>{},
    )
